  ...
  return (
    <div className="relative">
      <button onClick={() => setOpen((v) => !v)} className="flex items-center gap-1 rounded-lg border border-[#d4af37]/20 px-2.5 py-1.5 text-xs text-[#d4af37] transition hover:border-[#d4af37]/50">
        <Star className="h-3.5 w-3.5" /> {t('wdg.watchlist')}
      </button>
      {open && (
        <>
          ...
          <div className="absolute right-0 z-50 mt-1 w-48 rounded-xl border border-[#d4af37]/20 bg-[#0f0f14] p-1.5 shadow-xl">
            ...
            {lists.map((l) => {
              ...
              return (
                <button key={l.id} disabled={has} onClick={() => handleAdd(l.id)} title={has ? t('wl.added') : t('wl.addToWl')} className={`flex w-full items-center justify-between rounded-lg px-2 py-1.5 text-xs transition ${has ? 'text-emerald-400' : 'text-[#e9e7df] hover:bg-white/5'}`}>
