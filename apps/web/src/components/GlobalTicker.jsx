...
export default function GlobalTicker() {
  const { tickers, status } = useMarketData();
  ...
  return (
    <div className="fixed inset-x-0 top-0 z-50 h-[var(--header-h)] border-b border-[#d4af37]/10 bg-[#0a0a0f]/95 backdrop-blur-sm" style={{ paddingTop: 'var(--safe-top)' }}>
      <div className="flex h-5 items-center gap-2 px-4 text-[10px] text-[#8a8577] sm:px-6">
        <span className={`h-1.5 w-1.5 shrink-0 rounded-full ${wsConnected ? 'bg-emerald-400 animate-pulse' : ws === 'offline' || status === 'error' ? 'bg-red-400' : 'bg-[#d4af37]'}`} />
        <span className="truncate">{wsConnected ? 'Real-time · WebSocket' : ws === 'offline' ? 'Offline' : ws === 'reconnecting' ? 'Reconnecting…' : status === 'live' ? 'Live markets' : 'Loading…'}</span>
      </div>
      <div className="h-11 overflow-hidden border-t border-[#d4af37]/10">
