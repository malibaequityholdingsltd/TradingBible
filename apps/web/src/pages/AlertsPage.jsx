  ...
  return (
    <div className="fixed inset-0 z-50 flex items-start justify-center bg-black/70 p-4 pt-20" onClick={onClose}>
      <div className="glass w-full max-w-md rounded-2xl p-5" onClick={(e) => e.stopPropagation()}>
        ...
        <button onClick={submit} className="w-full rounded-lg bg-gradient-to-r from-[#f4e6a8] to-[#c99a25] py-2.5 text-sm font-semibold text-[#0a0a0f]">{t('al.create')}</button>
      </div>
    </div>
  );
}
  ...
  return (
    <AppLayout title={t('nav.alerts')}>
      <div className="mb-4 grid grid-cols-2 gap-3 sm:grid-cols-4">
        {[{ l: t('al.active'), v: activeCount, c: 'text-emerald-400' }, { l: t('al.paused'), v: pausedCount, c: 'text-[#d4af37]' }, { l: t('al.triggered'), v: triggeredCount, c: 'text-blue-400' }, { l: t('al.history'), v: history.length, c: 'text-[#c9c4b4]' }].map((s) => (
      ...
      <div className="mb-4 flex items-center gap-2">
        <div className="flex overflow-hidden rounded-lg border border-[#d4af37]/15">
          <button onClick={() => setTab('active')} className={`px-4 py-2 text-sm ${tab === 'active' ? 'bg-[#d4af37]/20 text-[#d4af37]' : 'text-[#8a8577]'}`}>{t('al.alerts')}</button>
          <button onClick={() => setTab('history')} className={`flex items-center gap-1 px-4 py-2 text-sm ${tab === 'history' ? 'bg-[#d4af37]/20 text-[#d4af37]' : 'text-[#8a8577]'}`}><History className="h-3.5 w-3.5" /> {t('al.notifications')}</button>
        </div>
        <button onClick={() => setCreating(true)} className="ml-auto flex items-center gap-1.5 rounded-lg bg-gradient-to-r from-[#f4e6a8] to-[#c99a25] px-4 py-2 text-sm font-semibold text-[#0a0a0f]"><Plus className="h-4 w-4" /> {t('al.newBtn')}</button>
      </div>

      {loading ? (
        <div className="glass flex items-center justify-center gap-2 rounded-2xl py-20 text-sm text-[#8a8577]"><Loader2 className="h-4 w-4 animate-spin" /> {t('c.loading')}</div>
      ) : tab === 'active' ? (
        alerts.length === 0 ? (
          <div className="glass rounded-2xl py-16 text-center"><Bell className="mx-auto mb-3 h-8 w-8 text-[#d4af37]/60" /><p className="text-sm text-[#8a8577]">{t('al.none')}</p></div>
        ) : (
        ...
        history.length === 0 ? (
          <div className="glass rounded-2xl py-16 text-center"><History className="mx-auto mb-3 h-8 w-8 text-[#d4af37]/60" /><p className="text-sm text-[#8a8577]">{t('al.noHist')}</p></div>
        ) : (
