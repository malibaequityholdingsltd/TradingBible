...
const HERO_STATS = [
  { value: '8', key: 'land.s1' },
  { value: '14', key: 'land.s2' },
  { value: '99.9%', key: 'land.s3' },
  { value: '1', key: 'land.s4' },
];
    ...
    <header className={`fixed inset-x-0 top-[var(--header-h)] z-40 border-b transition-all duration-300 ${scrolled ? 'border-[#d4af37]/15 bg-[#07070a]/92 shadow-[0_8px_32px_rgba(0,0,0,0.35)] backdrop-blur-xl' : 'border-[#d4af37]/10 bg-[#07070a]/70 backdrop-blur-xl'}`}>
      <div className="mx-auto flex w-full max-w-[96rem] items-center justify-between gap-4 px-4 py-3 sm:px-6 sm:py-4">
        ...
        <nav className="hidden items-center gap-1 rounded-full border border-white/5 bg-white/[0.03] p-1.5 md:flex">
          ...
          <a href="#ai" className={navLink}>{t('land.aiCoach')}</a>
      ...
      {menuOpen && !isAuthed && (
        <div className="border-t border-[#d4af37]/10 bg-[#07070a]/95 px-4 py-3 backdrop-blur-xl md:hidden">
          <nav className="flex flex-col gap-1">
            ...
            <a href="#ai" onClick={() => setMenuOpen(false)} className="rounded-xl px-4 py-3 text-sm font-medium text-[#8a8577] transition-colors hover:bg-white/5 hover:text-[#f0ecdd]">{t('land.aiCoach')}</a>
...
const FEATURES = [
  { icon: BarChart3, tk: 'land.f1t', bk: 'land.f1b' },
  { icon: Bot, tk: 'land.f2t', bk: 'land.f2b' },
  { icon: BookOpen, tk: 'land.f3t', bk: 'land.f3b' },
  { icon: Plug, tk: 'land.f4t', bk: 'land.f4b' },
  { icon: Zap, tk: 'land.f5t', bk: 'land.f5b' },
  { icon: Shield, tk: 'land.f6t', bk: 'land.f6b' },
];
    ...
    <div className="relative min-h-screen overflow-hidden bg-transparent text-[#e9e7df]">
      ...
      <section className="relative flex flex-col justify-center overflow-hidden pb-16 pt-[calc(11rem+var(--safe-top))] sm:pt-[calc(13rem+var(--safe-top))] lg:min-h-[92dvh]">
        ...
        <div className="relative mx-auto flex w-full max-w-[72rem] flex-col items-center px-6 text-center">
          ...
          <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7, delay: 0.24 }} className="mt-10 flex flex-col gap-3.5 sm:flex-row sm:items-center sm:gap-4">
            ...
            <Link to="/pricing" className="btn-glass inline-flex items-center justify-center rounded-xl border border-[#d4af37]/35 bg-white/[0.02] px-8 py-4 text-base font-semibold text-[#e9e7df] transition hover:border-[#d4af37]/60 hover:text-[#e9e7df]">{t('land.viewPricing')}</Link>
          </motion.div>
            ...
            <div className="grid grid-cols-2 gap-6 sm:grid-cols-4">
              {HERO_STATS.map((s) => (
                <div key={s.key}>
                  ...
                  <div className="mt-1 text-xs uppercase tracking-[0.18em] text-[#8a8577]">{t(s.key)}</div>
                </div>
              ))}
            </div>
          </motion.div>
      ...
      <section id="features" className="mx-auto max-w-[96rem] px-6 py-24">
        <div className="mb-14 max-w-2xl">
          <p className="mb-3 text-sm font-medium uppercase tracking-[0.2em] text-[#d4af37]">{t('land.featKick')}</p>
        ...
        <div className="grid gap-5 md:grid-cols-2 lg:grid-cols-3">
          {FEATURES.map((f, i) => (
            <motion.div key={f.tk} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.06 }} className="portfolio-card glass rounded-2xl p-6">
              <div className="mb-4 grid h-11 w-11 place-items-center rounded-xl bg-[#d4af37]/12 text-[#d4af37]"><f.icon className="h-5 w-5" /></div>
              <h3 className="portfolio-card__title mb-2 text-lg font-semibold">{t(f.tk)}</h3>
              <p className="text-sm leading-relaxed text-[#8a8577]">{t(f.bk)}</p>
            </motion.div>
          ))}
        </div>
      </section>
      ...
      <section className="mx-auto max-w-[96rem] px-6 py-24">
        <div className="mb-12 text-center">
          ...
          <p className="mt-3 text-[#8a8577]">{t('land.pricingSub')}</p>
        </div>
