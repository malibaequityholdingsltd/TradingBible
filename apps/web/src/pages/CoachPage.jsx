      ...
      <div className="grid gap-5 lg:grid-cols-3">
        <div className="glass flex h-[calc(100dvh-11rem)] min-h-[420px] flex-col rounded-2xl lg:col-span-2">
          <div className="flex items-center gap-2 border-b border-[#d4af37]/12 px-4 py-3 sm:px-5 sm:py-4">
            ...
            <span className="font-semibold text-[#f0ecdd]">{t('coach.tradingCoach')}</span>
            <span className="ml-auto flex items-center gap-1.5 text-xs text-emerald-400"><span className="h-2 w-2 rounded-full bg-emerald-400" /> {t('coach.online')}</span>
          </div>
          <div className="flex-1 space-y-4 overflow-y-auto p-4 sm:p-5">
            ...
            {empty && (
              <div className="rounded-2xl glass px-4 py-3 text-sm leading-relaxed text-[#e9e7df]">{t('coach.welcome')}</div>
            )}
            {messages.map((m, i) => (
          ...
          <div className="border-t border-[#d4af37]/12 p-3 sm:p-4">
            <div className="flex items-center gap-2 rounded-xl border border-[#d4af37]/15 bg-[#0f0f14] px-3 py-2">
              <input value={input} onChange={(e) => setInput(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && send()} disabled={isStreaming} placeholder={t('coach.askPh')} className="w-full bg-transparent text-sm text-[#e9e7df] placeholder-[#6a665a] outline-none disabled:opacity-60" />
        ...
        <div className="space-y-4">
          <div className="glass rounded-2xl p-5">
            <div className="mb-3 flex items-center gap-2 text-[#d4af37]"><Sparkles className="h-4 w-4" /><span className="text-sm font-semibold">{t('coach.suggested')}</span></div>
            <div className="space-y-2">{SUGGESTED_KEYS.map((k) => <button key={k} onClick={() => send(t(k))} disabled={isStreaming} className="w-full rounded-lg border border-[#d4af37]/12 px-3 py-2.5 text-left text-sm text-[#c9c4b4] transition hover:border-[#d4af37]/40 hover:text-[#f0ecdd] disabled:opacity-60">{t(k)}</button>)}</div>
          </div>
          <div className="glass rounded-2xl p-5">
            <h3 className="mb-3 text-sm font-semibold text-[#f0ecdd]">{t('coach.how')}</h3>
